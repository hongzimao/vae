...
class ConditionalVariationalAutoencoder(object):
    ...
    def __init__(self, sess, s_dim, c_dim, hidden_dim, learning_rate=0.001):

        self.sess = sess
        self.s_dim = s_dim  # input dimension
        self.c_dim = c_dim  # conditional input dimension
        self.hidden_dim = hidden_dim
        self.learning_rate = learning_rate

        self.inputs = tf.placeholder(tf.float32, [None, self.s_dim])
        self.cond_inputs = tf.placeholder(tf.float32, [None, self.c_dim])
    ...
    def train(self, inputs, cond_inputs):
        opt, loss = self.sess.run((self.opt, self.loss), feed_dict={
            self.inputs: inputs,
            self.cond_inputs: cond_inputs
        })
        return loss
    
    def encode(self, inputs, cond_inputs):
        return self.sess.run((self.enc_mean, self.enc_log_sigma_sq), feed_dict={
            self.inputs: inputs,
            self.cond_inputs: cond_inputs
        })
    
    def generate(self, cond_inputs, hidden_sample=None):
